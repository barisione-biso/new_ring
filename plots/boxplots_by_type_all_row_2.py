...
labels_ = ['URing', 'URing Adaptive', 'CompactLTJ', 'RDFCSA', 'Qdag BFS', 'MillenniumDB', 'Emptyheaded', 'Jena-LTJ', 'RDF3X', 'Virtuoso', 'Blazegraph']
colors = ['red', 'orange', 'yellow', 'green', 'lightblue', 'brown', 'blue', 'darkgray', 'tan', 'olive', 'lime']
k = 0
max_rows=1
max_columns=6
...
while k < len(query_types):
    ...
    new_order = ['backward_only', 'backward_only_adaptive', 'sigmod21', 'sigmod21_adaptive', 'one_ring_muthu_leap', 'one_ring_muthu_leap_adaptive', 'backward_only_muthu', 'backward_only_adaptive_muthu']
    data = data.reindex(columns=new_order)
    #Ring variants with better performance must remain ('URing', 'URing Adaptive'), the rest have to be removed.
    del data['sigmod21']
...
plt.savefig('boxplots_by_type_all_row_2.pdf', format='pdf',dpi=500)
plt.show()
