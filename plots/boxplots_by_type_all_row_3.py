...
query_types = ['Tr1', 'Tr2', 'S1', 'S2', 'S3', 'S4']

labels_ = ['Ring Adaptive Muthu', 'URing Adaptive', 'CompactLTJ', 'RDFCSA', 'Qdag BFS', 'MillenniumDB', 'Emptyheaded', 'Jena-LTJ', 'RDF3X', 'Virtuoso', 'Blazegraph']
colors = ['black', 'orange', 'yellow', 'green', 'lightblue', 'brown', 'blue', 'darkgray', 'tan', 'olive', 'lime']
k = 0
max_rows=1
max_columns=6
...
while k < len(query_types):
    ...
    new_order = [ 'one_ring_muthu_leap_adaptive', 'backward_only_adaptive', 'sigmod21', 'sigmod21_adaptive', 'one_ring_muthu_leap', 'backward_only','backward_only_muthu', 'backward_only_adaptive_muthu']
    data = data.reindex(columns=new_order)
    #Ring variants with better performance must remain ('Ring Adaptive Muthu', 'URing Adaptive'), the rest have to be removed.
    del data['sigmod21']
    del data['sigmod21_adaptive']
    del data['one_ring_muthu_leap']
    del data['backward_only']
    del data['backward_only_muthu']
    del data['backward_only_adaptive_muthu']
...
plt.savefig('boxplots_by_type_all_row_3.pdf', format='pdf',dpi=500)
plt.show()
